...
其中，submitter目录名为提交公司或组织的名称；system 目录名只能为 Linux 或者 Windows；modelx目录名只能为评测模型名称或模型名称+深度学习框架名称，每次提交至少包含1个system目录，system目录中可以只提交部分模型；code目录中存放评测代码；data目录用于存放评测使用的标准数据集；log目录用于存放评测日志结果，每次提交时，单机单卡、单机8卡和4机32卡评测日志至少提交一个。下面详细说明必要文件的内容和格式

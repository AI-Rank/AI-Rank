...
- 模型使用规范
    - 原则上不能使用预训练模型，模型初始化按照常量或随机分布进行；对于必须使用预训练模型的任务，需要说明预训练模型的出处，并提供详细的信息或代码保证预训练模型可复现。例如，Mask RCNN模型加载ResNet50预训练模型进行训练，需要说明ResNet50预训练模型出处（[原版实现](https://github.com/KaimingHe/deep-residual-networks)，torchvision等），并提供详细信息保证预训练权重可复现。
...
- 原则上不能使用预训练模型进行初始化。模型参数的初始化使用常量初始化或者某个分布初始化。对于必须使用预训练模型的任务，需要说明预训练模型的出处，并提供详细的信息或代码保证预训练模型可复现
- 训练结果可复现。不可复现的训练结果无效。

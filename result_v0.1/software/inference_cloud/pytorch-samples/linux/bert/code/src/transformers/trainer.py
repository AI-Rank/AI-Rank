...
class Trainer:
    ...
    def prediction_loop(
        ...
        if not self.args.ai_rank_logging:
            rank_logger.disable()
        rank_logger.info('load_data, checksum:xxxxxxxxxx')
        rank_logger.info('test_begin')
        rank_logger.info('warmup_begin, warmup_samples:{}'.format(self.args.warmup_times))
        warmup_times = 0
        tmp_inputs = {}
        cnt = 0
        correct_cnt = 0
        ...
        for step, inputs in enumerate(dataloader):
            ...
            while warmup_times < self.args.warmup_times:
                ...
                if warmup_times == self.args.warmup_times:
                    logits.to('cpu') #ensure warmup is done
                    del tmp_inputs
            ...
            if cnt > num_examples:
                cnt = num_examples
            # calculate accuracy if run on validation set
            if labels != None and self.args.ai_rank_logging:
                t1 = np.argmax(logits.cpu().numpy(), axis=1)
                t2 = labels.cpu().numpy()
                correct_cnt = correct_cnt + (t1 == t2).sum().item()
                rank_logger.info('total_accuracy:{}, total_samples_cnt:{}'.format(float(correct_cnt / cnt), cnt))

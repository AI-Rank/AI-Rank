...
from .ai_rank_logger import ai_logger
...
class Trainer:
    ...
    def prediction_loop(
        ...
        # warm up
        if not self.args.ai_rank_logging:
            self.rank_logger.disable()
        self.rank_logger.info('test_begin')
        self.rank_logger.info('warmup_begin, warmup_samples:{}'.format(self.args.warmup_times))
        warmup_times = 0
        tmp_inputs = {}
        cnt = 0
        ...
        for step, inputs in enumerate(dataloader):
            # ai rank warm up
            while warmup_times < self.args.warmup_times:
                if warmup_times == 0:
                    warmup_start = time.time()
                    for k in inputs:
                        tmp_inputs[k] = inputs[k].clone()
                loss, logits, labels = self.prediction_step(model, tmp_inputs, prediction_loss_only, ignore_keys=ignore_keys)
                warmup_times = warmup_times + 1
                if warmup_times == self.args.warmup_times:
                    logits[1].to('cpu') #ensure warmup is done
                    del tmp_inputs
                    self.rank_logger.info('warmup_finish')
                    warmup_end = time.time()
                    self.warmup_duration = warmup_end - warmup_start
            ...
            if self.args.ai_rank_logging:
                #t1 = np.argmax(logits.cpu().numpy(), axis=1)
                #t2 = labels.cpu().numpy()
                #correct_cnt = correct_cnt + (t1 == t2).sum().item()
                self.rank_logger.info('total_samples_cnt:{}'.format(cnt))
        ...
        #rank_logger.info('avg_ips:{}samples/sec'.format(float(num_examples / (infer_end - infer_start + warmup_start - warmup_end ))))
        #rank_logger.info('test_end')

...
def main(distributed_world_size,rank):
    ...
    if rank == 0:
        print("- AI-Rank-log %.3f load_data, checksum:%d" %(time.time(),len(train_dataset)))
    
    data_len_onerank =len(train_dataset) // distributed_world_size
    ...
    if rank == 0:
        sss = time.time()
        print("- AI-Rank-log %.3f test_begin" %(sss))     
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if  rank == 0 and ((epoch + 1) % args.eval_freq == 0 or epoch == args.epochs - 1):
            ...
            print("- AI-Rank-log %.3f eval_accuracy:%f, total_epoch_cnt:%d" %(time.time(),prec1,epoch))
            ...
            print(output_best)
            if best_prec1 >= 70.0:
                print("- AI-Rank-log %.3f target_quality_time:%.2fsec" %(time.time(),time.time() - sss))
                import sys
                sys.exit(0)
    ...
    if rank == 0:
        eee = time.time()
        print("- AI-Rank-log %.3f test_finish" %(eee))  
        print("- AI-Rank-log %.3f total_use_time:%.2fsec"%(time.time(),eee-sss))
        print("- AI-Rank-log %.3f avg_ips:%d videos/sec" %(time.time(),int(args.epochs * data_len_onerank / (eee-sss))))
...
def run():
    ...
    main(distributed_world_size,distributed_rank)

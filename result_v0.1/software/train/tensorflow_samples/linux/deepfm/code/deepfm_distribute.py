    ...
    if role == "PSERVER":
        ...
        with tf.device(device_setter):
            ...
            dense_w_one = tf.get_variable(
                "dense_w_one", [13], tf.float32, initializer=tf.truncated_normal_initializer(mean=0.0,
                                                                                             stddev=init_value / math.sqrt(float(FLAGS.embedding_size))))
            dense_w = tf.get_variable(
                "dense_w", [1, 13, 10], tf.float32, initializer=tf.truncated_normal_initializer(mean=0.0,
                                                                                                stddev=init_value / math.sqrt(float(FLAGS.embedding_size))))

            # -------------------- first order term  --------------------
            sparse_embedding_one = tf.concat(sparse_one_word, axis=1)
            sparse_embedding_one = tf.reshape(
                sparse_embedding_one, [-1, 26, 1])
            ...
            y_second_order = 0.5 * \
                tf.reduce_sum(summed_features_square -
                              squared_sum_features_emb, 1)
            ...
            cost = tf.losses.log_loss(label_y, predict, epsilon=1e-4)
